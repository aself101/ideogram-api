...
import type { GenerationResponse, DescribeResponse, ImageData } from './types/index.js';
import {
  getIdeogramApiKey,
...
try {
  ...
  try {
    // Fallback to src location (when running with tsx)
    const pkgPath = path.join(__dirname, '..', '..', 'package.json');
    const pkg = JSON.parse(readFileSync(pkgPath, 'utf8'));
    version = pkg.version;
  } catch {
...
/**
 * Generate command options interface.
 */
...
/**
 * Edit command options interface.
 */
...
/**
 * Remix command options interface.
 */
...
/**
 * Reframe command options interface.
 */
...
/**
 * Replace background command options interface.
 */
...
/**
 * Upscale command options interface.
 */
...
/**
 * Execute operation configuration interface.
 */
interface ExecuteOperationConfig {
  operation: string;
  spinnerMessage: string;
  successMessage: string;
  failureMessage: string;
  validateParams?: Record<string, unknown>;
  apiMethodCaller: (api: IdeogramAPI) => Promise<GenerationResponse | DescribeResponse>;
  resultHandler: (
    response: GenerationResponse | DescribeResponse,
...
async function executeOperation({
  ...
  resultHandler,
  globalOptions,
}: ExecuteOperationConfig): Promise<void> {
  try {
    setLogLevel(globalOptions.logLevel);
    const apiKey = getIdeogramApiKey(globalOptions.apiKey || null);
    const api = new IdeogramAPI(apiKey, undefined, globalOptions.logLevel);

    // Pre-flight validation (optional) - throws if invalid
    if (validateParams) {
...
program
  ...
  .option(
    ...
    `Rendering speed (${RENDERING_SPEEDS.join(', ')})`,
    'DEFAULT'
  )
  ...
  .action(async (options: GenerateOptions) => {
    await executeGenerate(options, program.opts() as GlobalOptions);
  });
...
program
  ...
  .action(async (options: EditOptions) => {
    await executeEdit(options, program.opts() as GlobalOptions);
  });
...
program
  ...
  .option('--style-type <type>', 'Style type')
  .option('--style-preset <preset>', 'Style preset name')
  .action(async (options: RemixOptions) => {
    await executeRemix(options, program.opts() as GlobalOptions);
  });
...
program
  ...
  .option('--rendering-speed <speed>', 'Rendering speed', 'DEFAULT')
  .option('--style-preset <preset>', 'Style preset name')
  .action(async (options: ReframeOptions) => {
    await executeReframe(options, program.opts() as GlobalOptions);
  });
...
program
  ...
  .option('--rendering-speed <speed>', 'Rendering speed', 'DEFAULT')
  .option('--style-preset <preset>', 'Style preset name')
  .action(async (options: ReplaceBackgroundOptions) => {
    await executeReplaceBackground(options, program.opts() as GlobalOptions);
  });
...
program
  ...
  .option('--seed <number>', 'Random seed')
  .action(async (options: UpscaleOptions) => {
    await executeUpscale(options, program.opts() as GlobalOptions);
  });
...
program
  ...
  .action(async (options: DescribeOptions) => {
    await executeDescribe(options, program.opts() as GlobalOptions);
  });
...
async function executeGenerate(
  ...
  try {
    ...
    for (let i = 0; i < prompts.length; i++) {
      ...
      // Pre-flight validation - throws if invalid
      validateOperationParams('generate-v3', {
        ...
        numImages: parseInt(options.numImages || '1'),
      });
      ...
      try {
        const response = await api.generate({
          ...
          resolution: options.resolution,
          aspectRatio: options.aspectRatio,
          renderingSpeed: options.renderingSpeed as
            | 'FLASH'
            | 'TURBO'
            | 'DEFAULT'
            | 'QUALITY'
            | undefined,
          magicPrompt: options.magicPrompt as 'AUTO' | 'ON' | 'OFF' | undefined,
          negativePrompt: options.negativePrompt,
          numImages: parseInt(options.numImages || '1'),
          seed: options.seed ? parseInt(options.seed) : undefined,
          colorPalette: options.colorPalette,
          styleType: options.styleType as
            | 'AUTO'
            | 'GENERAL'
            | 'REALISTIC'
            | 'DESIGN'
        ...
        await saveResults(response, 'generate-v3', prompt, globalOptions.outputDir, {
          ...options,
          prompt: [prompt],
        });
      } catch (error) {
 ...
 */
async function executeEdit(options: EditOptions, globalOptions: GlobalOptions): Promise<void> {
 ...
 */
async function executeRemix(options: RemixOptions, globalOptions: GlobalOptions): Promise<void> {
  await executeOperation({
    operation: 'remix-v3',
 ...
 */
async function executeReplaceBackground(
  ...
  await executeOperation({
    ...
    apiMethodCaller: (api) =>
      api.upscale({
        ...
        resemblance: options.resemblance ? parseInt(options.resemblance) : undefined,
        detail: options.detail ? parseInt(options.detail) : undefined,
        magicPromptOption: options.magicPromptOption as 'AUTO' | 'ON' | 'OFF' | undefined,
        numImages: parseInt(options.numImages || '1'),
        seed: options.seed ? parseInt(options.seed) : undefined,
      }),
    resultHandler: (response, globalOpts) =>
...
async function executeDescribe(
  ...
  await executeOperation({
    ...
    apiMethodCaller: (api) =>
      api.describe({
        image: options.image,
        describeModelVersion: options.describeModelVersion as 'V_2' | 'V_3' | undefined,
      }),
    resultHandler: async (response, globalOpts) => {
      const descriptions = extractDescriptions(response as DescribeResponse);
...
async function saveResults(
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    // Generate filename
    const filenameBase = images.length > 1 ? `${promptOrName}-${i + 1}` : promptOrName;
  ...
  await saveMetadata(metadataPath, {
    ...
    response: {
      ...
      images: images.map((img: ImageData, idx: number) => ({
        index: idx + 1,
        url: img.url,
